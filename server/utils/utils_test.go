...
func TestIsJiraAccessible(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Accessible instance":                   {`{"state": "RUNNING"}`, http.StatusOK, true},
		"Instance is not in correct state":      {`{"state": "STARTING"}`, http.StatusOK, false},
		"Instance is not returning status body": {"", http.StatusOK, false},
		"Instance not found":                    {`{"state": "RUNNING"}`, http.StatusNotFound, false},
	}
